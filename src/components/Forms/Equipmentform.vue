		...
		<div class="am-form froms am-form-horizontal">
			<div class="am-u-sm-12">
				<div class="am-form-group am-u-sm-6">
					...
					<div class="am-u-sm-8">
						<input type="text" v-model="forms.code" class="am-radius" placeholder="设备代码不可重复">
					</div>
				</div>
				<div class="am-form-group am-u-sm-6">
					<label for="doc-ipt-3" class="am-u-sm-4 am-form-label"><span>*</span>设备名称</label>
					<div class="am-u-sm-8">
						<input type="text" v-model="forms.name" class="am-radius" placeholder="请输入设备名称">
					</div>
				</div>
				<div class="am-form-group am-u-sm-6">
				...
				<div class="am-form-group">
					<label for="doc-ipt-3" class="am-u-sm-2 am-form-label">设备描述</label>
		...
		methods: {
			...
			save () {
				...
				if (!$.trim(self.forms.code)) {
					Public.dialog('请输入设备编码');
					return false;
				};
				if (!$.trim(self.forms.name)) {
					Public.dialog('请输入设备名称');
					return false;
				};
			...
			selectHtm (type, list) {
				...
				if (type == 1) {
					var title = '适应症';
					var arr = [];
					$.each(self.forms.equipment_indications_labels, function(index, val) {
						 arr.push(val.id);
					});
				} else {
					var title = '禁忌症';
					var arr = [];
					$.each(self.forms.equipment_contraindications_labels, function(index, val) {
						 arr.push(val.id);
					});
				}
				var item = '';
